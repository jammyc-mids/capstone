...
for h in houses:
   ...
   for idx, entry in house_data.iterrows():
      ...
      if not (mon == 5 and day in [1,2]):
         ...
         if resp.status_code == 200:
             print(f"Prediction request posted successfully to [{murl}]")
         else:
             print(f"Prediction request failed [{murl}]")
      ...
      ghi = get_irradiance_sequence(irradiance, f"{year}-{mon}-{day}", hour, mins)
